...
import { loadServerConfig, type TopKDefaults, type TopKOverrides } from "./config.js";
...
let defaults: TopKDefaults = { ...serverCfg.defaults };
...
router.put("/v1/settings", async (_req, res, body) => {
  const b = body as Partial<TopKDefaults> | null;
  if (!b) {
    json(res, 400, { error: "Body required" });
    return;
  }
  if (b.retrieveTopK !== undefined) defaults.retrieveTopK = clampTopK(b.retrieveTopK);
  if (b.colbertTopK !== undefined) defaults.colbertTopK = clampTopK(b.colbertTopK);
  if (b.rerankTopK !== undefined) defaults.rerankTopK = clampTopK(b.rerankTopK);
...
router.post("/v1/search", async (_req, res, body) => {
  const b = body as SearchBody | null;
  if (!b?.query) {
  ...
  const handle = taskQueue.submit(
    () => pipeline.run(b.query, opts),
    priority,
  );
...
router.post("/v1/search/urgent", async (_req, res, body) => {
  ...
  const handle = taskQueue.submitUrgent(
    () => pipeline.run(b.query, opts),
  );
...
router.post("/v1/embed", async (_req, res, body) => {
  const b = body as { text?: string | string[]; priority?: number } | null;
  if (!b?.text) {
...
router.post("/v1/rerank", async (_req, res, body) => {
  const b = body as RerankBody | null;
...
interface RerankBody {
  query: string;
  documents: Array<{ id: string | number; content: string; metadata?: Record<string, unknown> }>;
  priority?: number;
  rerank_top_k?: number;
  colbert_top_k?: number;
  two_stage?: boolean;
...
function buildPipelineOptions(b: SearchBody): PipelineOptions {
  const opts: PipelineOptions = {
    retrieveTopK: b.retrieve_top_k ?? defaults.retrieveTopK,
    colbertTopK: b.colbert_top_k ?? defaults.colbertTopK,
    rerankTopK: b.rerank_top_k ?? defaults.rerankTopK,
  };
  if (b.skip_colbert !== undefined) opts.skipColbert = b.skip_colbert;
  if (b.collection !== undefined) opts.collection = b.collection;
  if (b.filter !== undefined) opts.filter = b.filter;
  return opts;
}
